...
func LoadConfig(filePath string) (*Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error opening configs file: %w", err)
	}
	...
	if err := decoder.Decode(&config); err != nil {
		return nil, fmt.Errorf("error decoding configs file: %w", err)
	}
	return &config, nil
}

// GetConfigFilePath returns the correct config file based on the environment
func (c *Config) GetConfigFilePath(string) string {
	return fmt.Sprintf("configs/config-%s.json")
}
...
func (c *Config) SetDefaults() {
	...
	if utils.IsEmptyString(c.AppEnv) {
		c.AppEnv = "dev" // Default to development
	}
}
